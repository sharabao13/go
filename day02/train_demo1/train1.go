...
func main() {
	...
	nums := []int{1, 2, 5, 20, 60, 80, 10, 100, 7, 22}
	maxNums, senondNums := nums[0], nums[0]
	for _, v := range nums {
		if v > maxNums {
			senondNums = maxNums
			maxNums = v
	...
	fmt.Println(maxNums, senondNums)
	//3. 获取映射中所有key组成的切片
	users := map[string]int{"tae": 10, "bao": 0, "mei": 8}
...
This is our hope. This is the faith that I will go back to the South with. With this faith we will be able to hew out of the mountain of despair a stone of hope. With this faith we will be able to transform the jangling discords of our nation into a beautiful symphony of brotherhood. With this faith we will be able to work together, to pray together, to struggle together, to go to jail together, to stand up for freedom together, knowing that we will be free one day. And this will be the day, this will be the day when all of God's children will be able to sing with new meaning, "My country 'tis of thee, sweet land of liberty, of thee I sing. Land where my fathers died, land of the Pilgrim's pride, from every mountainside, let freedom ring!" And if America is to be a great nation, this must become true.`
	...
	//6. 冒泡排序
	//同上
	sums := []int{5, 6, 100, 6, 122, 30, 80, 55}
	sort.Ints(sums)
	fmt.Println(sums)
	//7. 挑战插入游戏，
	//8. 跳转猜游戏(折半猜)
	sums1 := []int{1, 3, 5, 7, 9}
	fmt.Println(sort.SearchInts(sums1, 5))
	//在有序的数组中查找
}
